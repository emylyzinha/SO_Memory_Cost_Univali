...
// pthread_mutex_t q_lock = PTHREAD_MUTEX_INITIALIZER;  // removido
sem_t sem_items;  // quantas tarefas disponíveis
sem_t sem_space;  // espaços livres no buffer
sem_t sem_lock;   // semáforo binário para exclusão mútua (substitui mutex)

// Enfileirar tarefa (produtor)
void q_push(Task t) {
    sem_wait(&sem_space);               // espera espaço
    sem_wait(&sem_lock);                // exclusão mútua (substitui pthread_mutex_lock)
    qbuf[q_tail] = t;
    q_tail = (q_tail + 1) % QMAX;
    sem_post(&sem_lock);                // libera exclusão (substitui pthread_mutex_unlock)
...
Task q_pop() {
    ...
    sem_wait(&sem_lock);                // exclusão mútua
    Task t = qbuf[q_head];
    q_head = (q_head + 1) % QMAX;
    sem_post(&sem_lock);                // libera exclusão
...
int main(int argc, char** argv) {
    ...
    sem_init(&sem_lock, 0, 1);         // semáforo binário substituindo mutex
    ...
    // Garante que a pasta "img" exista e salva o arquivo lá usando apenas o basename
    mkdir("out", 0755); // ignora erro se já existir
    std::string outfile(outpath);
    size_t pos = outfile.find_last_of('/');
    std::string basename = (pos == std::string::npos) ? outfile : outfile.substr(pos + 1);
    std::string savepath = std::string("out/") + basename;

    cout << "Worker: processamento concluido. Salvando em " << savepath << "...\n";
    if (write_pgm(savepath.c_str(), &g_out) != 0) {
        cerr << "Erro ao salvar " << savepath << endl;
    } else {
        cout << "Worker: imagem salva em " << savepath << endl;
    }
